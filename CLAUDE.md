...
**Troubleshooting «Не вдалось відправити pack у репо»:** обмінний клон і routine обоє пишуть у `main`, тому клон може розійтися з origin (`git -C /root/max-parser-repo status` показує `ahead N, behind M`). Тоді `git pull --ff-only` падає (exit 128, «Not possible to fast-forward»), за ним push (exit 1) → кнопка віддавала 500. **Самолікується:** `_git_pull()` у `dashboard.py` ловить впалий ff-pull і робить `fetch` + `reset --hard origin/main` автоматично — origin канонічний (там `analytics/results/`), локальні housekeeping-коміти невдалих спроб відкидаються без втрат. Тобто симптом має зникнути сам із наступним кліком. Якщо все ж повторюється (наприклад reset теж падає — дивись `journalctl -u max-dashboard | grep analytics`) — ручний фікс той самий: `git -C /root/max-parser-repo reset --hard origin/main`.
