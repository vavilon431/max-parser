...
## [2026-06-17 17:12] — Фікс кнопки «Аналітика» (розсинхрон обмінного репо) + день тижня в графіку

**Контекст:** дві задачі за сесію. (1) Кнопка «🧠 Аналітика» падала з «Не вдалось відправити pack у репо. Перевірте логи дашборду». (2) Прохання: у графіку «Динаміка згадок» при наведенні показувати поряд із датою короткий день тижня (пн/вт/ср).

**Ключові зміни:**
- **Task 1 — кнопка «Аналітика» (серверний фікс, без правок коду):** обмінний git-клон `/root/max-parser-repo` розійшовся з origin (`ahead 7, behind 1`) — дашборд накопичив 7 локальних коммітів невдалих pack-спроб, а cloud-routine запушила 1 свій. Через розбіжність `git pull --ff-only` у `api_analytics_start` падав (exit 128, «Not possible to fast-forward»), за ним і push (exit 1) → 500. Фікс: `git -C /root/max-parser-repo reset --hard origin/main` (origin канонічний — там results; локальні 6 housekeeping-коммітів невдалих спроб відкинуто без втрат). Після reset `pull --ff-only` → «Already up to date».
- **Task 2 — день тижня в графіку (dashboard.py):** tooltip «Динаміки згадок» тепер показує `27 кві 2026 · сб`. Додано `_WEEKDAYS_UA`, функцію `weekdayUA(iso)` (через `Date.UTC`, без TZ-зсуву), оновлено `tooltip.callbacks.title`.
- **Деплой:** `scp dashboard.py` + `systemctl restart max-dashboard` — active, лог після рестарту чистий, HTTP 302.
- **CLAUDE.md:** у секцію «AI-аналітика через cloud routine» додано troubleshooting-абзац про симптом «Не вдалось відправити pack» = розсинхрон клону з origin і фікс через `reset --hard origin/main`.

**Поточний стан:** обидві задачі задеплоєно й перевірено на `max-vps`. Доступ до VPS — через WARP реконект-цикл (робочий exit-IP `104.28.156.62`; `.61` мертвий). Реальний клік «Аналітика» не робив, щоб не палити квоту routine — корінь усунено на рівні git. **`max-parser` (токен A) ВСЕ ЩЕ inactive з 23.05** — live тримає лише токен B + backfill.

**Наступний крок:**
1. Користувачу: клік «Аналітика» на проді — підтвердити, що pack тепер доходить (1 з 15 добових квот routine).
2. Підняти токен A (тягнеться з 23.05): `systemctl start max-parser` якщо токен живий, інакше QR-rotation через `pw_qr_a.py`.

---

## [2026-06-02 18:02] — AI-аналітика працює з порожнім пошуком (останні пости)

**Контекст:** користувач попросив, щоб кнопка «🧠 Аналітика» працювала і з порожнім полем пошуку — тоді аналіз іде по останніх постах (усього парсера або, якщо вибрано канал, тільки цього каналу).

**Ключові зміни:**
- [dashboard.py](../dashboard.py) `api_analytics_start`: знято блокування `q_required` — порожній `q` тепер дозволено. Вибірка йде через наявну гілку `_query_export_rows` без FTS (фільтри channel/period + `ORDER BY id DESC`).
- Захист пам'яті: константа `ANALYTICS_MAX_ROWS = 6000` + опціональний `limit` у `_query_export_rows`. Без слова вибірка за добу по ~3756 каналах інакше тягнула б сотні тисяч рядків у 800МБ-процес; `_build_analytics_input` однаково ріже на 600k символів, тож на результат це не впливає. XLSX-експорт лишився з повною вибіркою (`limit=None`).
- Заголовок блоку аналітики при порожньому пошуку показує «останні пости (всі канали)» / «останні пости: <канал>» замість порожніх лапок (FE `qLabel`). Прибрано мертвий `q_required` з FE-словника помилок. У метаданих паку `# q:` пишеться як «(порожній — останні пости)».
- [summary.txt](../summary.txt): системний промпт переписано під два режими — за ключовим словом (як було) і без нього (домінуючі наративи/тригери всього масиву). Виправлено друкарську «зыбрані».

**Поточний стан:** задеплоєно на `max-vps` (scp + `systemctl restart max-dashboard`), сервіс active, стартував чисто. Приймальний клік за користувачем (кожен запуск їсть 1 з 15 добових квот routine).

**Наступний крок:**
1. Користувачу: клік «Аналітика» з порожнім пошуком — перевірити результат на проді.
2. Незавершене з минулих етапів: перевипуск токена A (`pw_qr_a.py`, тягнеться з 23.05); зміна пароля `user1`.
